...
def main():
    ...
    image=Image.open('cropimg.jpg')
    st.image(image)
    html_temp = """
    <div style="background-color:teal; padding:10px">
    <h2 style="color:white;text-align:center;">Predict Most Suitable Crop</h2>
    </div>
    """

    st.markdown(html_temp, unsafe_allow_html=True)
    activities=['Naive Bayes (The Best Model)','Logistic Regression','Decision Tree','Random Forest']
    option=st.sidebar.selectbox("Which model you wnat to use?",activities)
    st.subheader(option)
